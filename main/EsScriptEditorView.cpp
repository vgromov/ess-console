...
{
  if( 
    EsScriptEditorView::acIdFunc == id || 
    EsScriptEditorView::acIdClassFunc == id || 
    EsScriptEditorView::acIdClassMemberFunc == id || 
    EsScriptEditorView::acIdVarsvc == id 
  )
  {
    EsString svc = item;
...
void EsScriptEditorView::operatorAndSubjGetLeftOfPos(int curPos, wxString& subj, wxString& op, bool searchSubj /*= true*/)
{
  ...
  ES_DEBUG_TRACE(
    esT("EsScriptEditorView::operatorAndSubjGetLeftOfPos(from: %d, subj: %s, op: %s, searchSubj: %d)"),
    curPos,
    esVC( subj.t_str() ),
    esVC( op.t_str() ),
    (int)searchSubj
...
wxString EsScriptEditorView::autocompletionListPrepare(int curPos, const EsString& filter, wxString subj, wxString op)
{
  ...
  ES_DEBUG_TRACE(
    esT("EsScriptEditorView::autocompletionListPrepare(curPos=%d, filter=%s, subj=%s, op=%s)"),
    curPos,
    filter,
    esVC(subj.t_str()),
    esVC(op.t_str())
  ...
  {
    ES_DEBUG_TRACE( 
      esT(" ..subj and op is empty - trying to find out local context at pos=%d.."),
      curPos
    );
  ...
  else
  {
    ...
    if( scriptedMetaInfo )
    {
      ...
      autocompletionItemsAppend(
        acItems,
        lookup->constantsFindSyntaxRestricted(
          filter
        ),
        acIdConst
      );
  ...
  for (size_t idx = 0; idx < acItems.size(); ++idx)
  {
    ...
    autoCompletionItems += acItems[idx].as<wxString>();
  }
...
void EsScriptEditorView::doCtxMenuStringsUpdate(wxMenu* pmnu)
{
  ...
  pmnu->SetLabel(
    EsSconsoleCmds::usrID_SEDIT_FILE_OPEN_AT_CURSOR,
    _("Open file\tCTRL-Enter").as<wxString>()
  );
  pmnu->SetHelpString(
    EsSconsoleCmds::usrID_SEDIT_FILE_OPEN_AT_CURSOR,
    _("Open script file at cursor").as<wxString>()
  );

  pmnu->SetLabel(
    EsSconsoleCmds::usrID_SEDIT_FILE_LOCATION_OPEN_AT_CURSOR,
    _("Open location\tALT-Enter").as<wxString>()
  );
  pmnu->SetHelpString(
    EsSconsoleCmds::usrID_SEDIT_FILE_LOCATION_OPEN_AT_CURSOR,
    _("Open location of file at cursor").as<wxString>()
  );

  pmnu->SetLabel(
    EsSconsoleCmds::usrID_SEDIT_LINE_COMMENT_TOGGLE,
    _("Toggle Line comment\tCTRL-/").as<wxString>()
  );
  pmnu->SetHelpString(
    EsSconsoleCmds::usrID_SEDIT_LINE_COMMENT_TOGGLE,
    _("Toggle line comment for current line or selection").as<wxString>()
  );

  pmnu->SetLabel(
    EsSconsoleCmds::usrID_SEDIT_BLOCK_COMMENT_TOGGLE,
    _("Toggle Block comment\tALT-/").as<wxString>()
  );
  pmnu->SetHelpString(
    EsSconsoleCmds::usrID_SEDIT_BLOCK_COMMENT_TOGGLE,
    _("Toggle block comment for selection").as<wxString>()
  );

  pmnu->SetLabel(
    EsSconsoleCmds::usrID_SEDIT_CODE_INSIGHT,
    _("Code Insight\tCTRL-Space").as<wxString>()
  );
  pmnu->SetHelpString(
    EsSconsoleCmds::usrID_SEDIT_CODE_INSIGHT,
    _("Open Code Insight browser console").as<wxString>()
  );
}
//--------------------------------------------------------------------------------
