...
void EsSconsoleOutputBase::ctxMenuInit()
{
  ...
  m_pmnu->Append(
    usrID_COPY,
    _("Copy\tCTRL-C"),
    _("Copy selected log contents to clipboard")
  );
  m_pmnu->AppendSeparator();
  m_pmnu->Append(
    usrID_SELECT_ALL,
    _("Select all\tCTRL-A"),
    _("Select all log contents")
  );
  m_pmnu->Append(
    usrID_CLEARALL,
    _("Clear all"),
    _("Clear entire log contents")
  );
  m_pmnu->AppendSeparator();
  m_pmnu->Append(
    usrID_GOTO,
    _("Goto location"),
    _("Goto specified source location")
  );
...
wxMenuPtr EsSconsoleOutputBase::ctxMenuGet()
{
  if(m_pmnu)
    return m_pmnu;
