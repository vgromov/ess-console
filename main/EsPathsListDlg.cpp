...
void EsPathsListDlg::onI18nLocaleChanged(const EsString& loc)
{
  ...
  m_btnUp->SetLabel(_("Up"));
  m_btnUp->SetToolTip(_("Move selected item up one row in the list"));

  m_btnDown->SetLabel(_("Down"));
  m_btnDown->SetToolTip(_("Move selected item dowm one row in the list"));

  m_btnAdd->SetLabel(_("Add"));
  m_btnAdd->SetToolTip(_("Add path, currently selected in folder picker, to the path list"));

  m_btnReplace->SetLabel(_("Replace"));
  m_btnReplace->SetToolTip(_("Replace currently selected item wuth one from the folder picker"));

  m_btnDelete->SetLabel(_("Delete"));
  m_btnReplace->SetToolTip(_("Delete currently selected item from the list"));

  m_lblList->SetLabel(
    EsStringI18n::translate(m_dirListLabel.t_str()).c_str()
  );

  m_contents->Layout();

  m_btnOk->SetLabel(_("OK"));
  m_btnCancel->SetLabel(_("Cancel"));
