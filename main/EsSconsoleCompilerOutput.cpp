...
void EsSconsoleCompilerOutput::onCompilationAborted(const EsString& input)
{
  textLineAppend(
    wxString::Format(
      _("Compilation of '%s' was interrupted by user"),
      input.c_str()
    )
  );
}
...
void EsSconsoleCompilerOutput::onCompilationEnd(const EsString& input, const EsTimeSpan& span, const EsString::Array& elog)
{
  if(elog.empty())
  {
    ...
    if(span.get_allWeeks() > 0)
      elapsed = span.asString(_("%Ew %Dd %H:%M:%S.%l")).c_str();
    else if(span.get_allDays() > 0)
      elapsed = span.asString(_("%Dd %H:%M:%S.%l")).c_str();
    else if(span.get_allHours() > 0)
      elapsed = span.asString(_("%H:%M:%S.%l")).c_str();
    else if(span.get_allMinutes() > 0)
      elapsed = span.asString(_("%M:%S.%l")).c_str();
    else
      elapsed = span.asString(_("%S.%l s")).c_str();

    textLineAppend(
      _("Compilation complete in: ") + elapsed
    );
  }
  else
  {
    textLineAppend(
      _("Error(s) occurred during compilation:")
    );
