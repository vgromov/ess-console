...
EsTextDocument::EsTextDocument(const EsAssocContainerIntf::Ptr& regInfo) :
EsDocument(regInfo)
{}
//--------------------------------------------------------------------------------

EsVariant EsTextDocument::NEW( cr_EsVariant regInfo )
{
  std::unique_ptr<EsTextDocument> ptr = ES_MAKE_UNIQUE(
    EsTextDocument,
    regInfo.asObject()
  );
  ES_ASSERT( ptr );
...
void EsTextDocument::emptyContentsInit(const EsVariant& ES_UNUSED(params))
{
...
void EsTextDocument::doWrite(cr_EsVariant key, cr_EsVariant params, bool asCopy)
{
  // Interpret src as script file path
